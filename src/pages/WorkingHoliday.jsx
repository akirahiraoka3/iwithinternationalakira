...
const WorkingHoliday = () => {
  const steps = [
    {
      title: "STEP1 ビザを申請する",
      content: "ワーキングホリデービザは国によって申請に必要な条件が異なります。条件には年齢制限や健康状態、十分な所持金などが含まれます。条件を満たしていないとビザを取得することができないので、必ず事前に行きたい国の申請条件を確認しておきましょう。\n申請には有効期限が十分なパスポートや英文の残高証明書などが必要になります。こちらも各国で異なりすぐに揃えることができないものばかりなので、事前に確認し準備しておきましょう。\n必要書類を揃えたら実際に申請をします。申請から取得までの手順はオンライン申請、書類郵送、面接、健康診断など国によって様々です。また、各国申請期間や抽選方法も異なるので必ず確認してから申請するようにしましょう。"
    },
    {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="container mx-auto px-4 py-16">
        ...
        <div key="step1" className="mb-8">
          <h3 className="text-xl font-semibold mb-4">STEP1 ビザを申請する</h3>
          <p>{steps[0].content}</p>
        </div>
        <div key="step2" className="mb-8">
          <h3 className="text-xl font-semibold mb-4">STEP2 必要な手配申請をする</h3>
          <p>{steps[1].content}</p>
        </div>
        <div key="step3" className="mb-8">
          <h3 className="text-xl font-semibold mb-4">STEP3 渡航した後に</h3>
          {afterArrival.map((item, index) => (
